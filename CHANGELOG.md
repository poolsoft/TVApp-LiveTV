...
* **Mobilde Dikey Mod (Portrait) Yerleşimleri:** `IptvSourcesActivity` (IPTV Liste Ekleme) ve `IptvChannelSelectionActivity` (Kanal Seçimi) ekranlarına özel `layout-port` dikey arayüz tasarımları eklendi; telefon dikey tutulduğunda listeler ve butonlar ekrana taşmadan mükemmel şekilde hizalanır. TV varyantları (`landscape`) bu dikey tasarımlardan tamamen yalıtılmıştır.
* **Kanal Seçiminde Buton Restorasyonu & Tıklanabilir Orijinal Efsane:** Eski dikey aksiyon çubuğu butonları (Tümünü Seç, Temizle, Kategori, Seçilenler, Kaydet) geri getirildi. Alt bardaki renk efsaneleri orijinal sade TV görünümüne (küçük renk noktaları ve sade metin) geri döndürülürken, dokunmatik tıklanabilirlikleri şeffaf dalgalanma efektiyle korundu.
* **Mobil Ana Ekrana Ayarlar Entegrasyonu:** Mobil arayüzde üst çubuğa (IPTV butonunun yanına) ve tam ekran OSD üst barına **Ayarlar Butonu** (`ic_settings`) eklendi; mobilde ayarlar ekranı dar TV overlay'i yerine tam ekran açılarak konforlu kullanım sağlandı.
* **Tam Ekran Yan Kanal Çekmecesi (Side Channel Drawer):** `MobileMainActivity` tam ekran (landscape) modundayken videodan çıkmadan sol üst bardaki kanal butonuyla açılabilen, yarı saydam şık yan kanal paneli eklendi. Kullanıcı canlı yayını izlemeye devam ederken kategori seçebilir, arama yapabilir, kanallar arasında gezinebilir ve tek dokunuşla kanal değiştirebilir.
...
* **Çoklu Manifest ve Ayrıştırılmış JSON Versiyonlama:** TV'nin mevcut güncelleme akışı (`version.json`) bozulmadan korunurken, mobil sürüm için bağımsız `version-mobile.json` ve kombine `packages` eşlemesi ana release akışına entegre edildi.
* **Dikey Mod (Portrait) & Tam Ekran:** Canlı IPTV oynatıcısı, anlık arama, kategori çipleri ve ses/parlaklık dokunmatik jest kontrolleri.
