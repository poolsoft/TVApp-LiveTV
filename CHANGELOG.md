...
* **Kanal Seçiminde Dokunmatik Aksiyon Çubuğu:** `IptvChannelSelectionActivity` ekranında yatay telefon ekranına sığmayan dikey kenar çubuğu mobilde gizlendi; alt bardaki renk efsaneleri (Temizle, Tümünü Seç, Kategori Filtresi, Sadece Seçilenler ve Seçimi Kaydet) dokunmatik, şık tıklanabilir buton gruplarına dönüştürüldü. TV'deki kumanda renk tuşları ve D-Pad odağı korunurken mobilde tek tıkla toplu işlem yapılması sağlandı.
* **Tam Ekran Yan Kanal Çekmecesi (Side Channel Drawer):** `MobileMainActivity` tam ekran (landscape) modundayken videodan çıkmadan sol üst bardaki kanal butonuyla açılabilen, yarı saydam şık yan kanal paneli eklendi. Kullanıcı canlı yayını izlemeye devam ederken kategori seçebilir, arama yapabilir, kanallar arasında gezinebilir ve tek dokunuşla kanal değiştirebilir. Geri tuşu ve ekrana dokunma ile çekmecenin yumuşak kapanışı sağlandı.
